=== Sree Saanvika Offers ===
Contributors: sreesaanvika
Tags: woocommerce, bogo, offers, discount, india
Requires at least: 6.0
Tested up to: 6.7
Requires PHP: 7.4
Stable tag: 1.0.0
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Buy 2 get 1 free, and offers like it, without a promo code.

== Description ==

Pick the products an offer covers. When a shopper has enough of them in the
cart, the cheapest ones come off the total on their own — no code to type, and
nothing for them to claim.

* Buy X get Y, at any percentage off. 100% is free; 50% makes the cheapest half
  price.
* Choose products by hand, or by whole category, with an exclusion list.
* As many offers as you like — one for sarees, another for jewellery, each with
  its own products.
* The banner shows on every product it covers and at the top of the cart, with
  a live line telling the shopper how many more to add.
* Optional countdown to the end of the offer.
* The free line is struck through in the cart and marked, so the discount is
  never a mystery at checkout.

Works with WooCommerce's High-Performance Order Storage. The discount is worked
out on the server, so it applies with JavaScript switched off.

== Installation ==

1. Upload the plugin and activate it.
2. Go to WooCommerce → Offers → Add offer.
3. Set buy 2, get 1, at 100% off. Pick your sarees, or the Sarees category.
4. Publish.

== Frequently Asked Questions ==

= Which item goes free? =

The cheapest of the qualifying items. With three sarees at ₹3,999, ₹2,999 and
₹1,200, the ₹1,200 one is free.

= What happens with six? =

Two free, if "apply it again for every further set" is on. Turn it off and one
set per cart is the limit.

= Can I run one for jewellery as well? =

Yes — add a second offer and pick jewellery instead. They are counted
separately, so two sarees and two bangles is not three of anything.

= Does it need a coupon code? =

No. There is nothing for the shopper to enter.
